...
BEGIN
    ...
    WITH items AS (
        SELECT 
            ...
            pa.account_holder_name AS payout_account_holder_name,
            pa.ifsc_code AS payout_ifsc_code,
            (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE id = pa.account_number_secret_id) AS payout_account_number_full,
            w.requested_at,
            w.approved_at,
            w.processing_at,
            w.paid_at,
