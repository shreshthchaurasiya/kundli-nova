-- DEFINITIVE FIX: Admin Dashboard Summary with correct column names
-- consultation_sessions: total_charged (not charge_amount), started_at (exists)
-- astrologer_withdrawal_requests: paid_at, failed_at (not processed_at)
...
BEGIN
    ...
    -- Consultation Stats
    SELECT COUNT(*) INTO v_active_consultations 
    FROM public.consultation_sessions 
    WHERE status IN ('WAITING_FOR_ASTROLOGER', 'ACTIVE');
    ...
    -- Reconciliation
    SELECT COUNT(*) INTO v_successful_payment_count 
    FROM public.payment_orders 
    WHERE status IN ('captured', 'credited') AND created_at >= p_from AND created_at < p_to;
    
    SELECT COALESCE(SUM(amount_paise / 100.0), 0) INTO v_razorpay_payment_volume 
    FROM public.payment_orders 
    WHERE status IN ('captured', 'credited') AND created_at >= p_from AND created_at < p_to;
    ...
    -- Include AI Subscription Revenue in Company Revenue
    v_company_commission_revenue := v_company_commission_revenue + v_ai_subscription_revenue;

    -- Payout & Withdrawals
    SELECT COUNT(*) INTO v_pending_payout_accounts 
    FROM public.astrologer_payout_accounts WHERE status = 'PENDING';
    
    SELECT COUNT(*) INTO v_pending_withdrawals 
    FROM public.astrologer_withdrawal_requests WHERE status = 'REQUESTED';
    
    SELECT COUNT(*) INTO v_processing_withdrawals 
    FROM public.astrologer_withdrawal_requests WHERE status IN ('APPROVED', 'PROCESSING');
    
    -- Using paid_at (correct column name, not processed_at)
    ...
    -- Using failed_at (correct column name)
    SELECT COUNT(*) INTO v_failed_withdrawals
    FROM public.astrologer_withdrawal_requests
    WHERE status = 'FAILED' AND failed_at >= p_from AND failed_at < p_to;
    ...
    -- Missing billing ledger: using total_charged (correct column, not charge_amount)
    ...
    WHERE cs.status = 'ENDED' AND cs.total_charged > 0 AND abl.id IS NULL;

    -- Stale active consultations: using started_at (exists)
    SELECT COUNT(*) INTO v_stale_active_consultation_count
    FROM public.consultation_sessions
    WHERE status = 'ACTIVE' AND started_at < (now() - interval '24 hours');

    v_result := jsonb_build_object(
        'users', jsonb_build_object('total', v_total_users),
        'astrologers', jsonb_build_object('total', v_total_astrologers, 'online', v_online_astrologers),
        'consultations', jsonb_build_object('active', v_active_consultations, 'completed', v_completed_consultations),
